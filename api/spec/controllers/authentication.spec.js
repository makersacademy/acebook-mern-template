...
describe("/tokens", () => {
  beforeAll(async () => {
    const user = new User({ name: "Perfect Person", email: "test@test.com", password: "12345678" })
  ...
  test("a token is returned when creds are valid", async () => {
    let response = await request(app)
      .post("/tokens")
      .send({name: "Perfect Person", email: "test@test.com", password: "12345678"})
  ...
  test("a token is not returned when creds are invalid", async () => {
    let response = await request(app)
      .post("/tokens")
      .send({name: "Perfect Person", email: "test@test.com", password: "1234"})
