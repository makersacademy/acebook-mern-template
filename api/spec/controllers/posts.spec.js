...
describe('/posts', () => {
  ...
  describe('GET, when token is present', () => {
    ...
    test('comments array is populated', async () => {
      const post = new Post({ user_id: user_id, message: 'howdy!' });
      let post_id = post._id;
      await post.save();
      comment1 = new Comment({
        user_id: user2_id,
        post_id: post_id,
        message: 'howdy to you too!',
      });
      await comment1.save();
      ...
      const response = await request(app)
        ...
        .send({ token: token });
      expect(
        response.body.posts[0].comments.map((comment) => comment.message)
      ).toEqual(['howdy to you too!', 'i like cheese']);
    });
  });
  ...
  describe('LIKES posts', () => {
    //need to create one then test it.
