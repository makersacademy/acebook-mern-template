  ...
  it("Calls the /comments endpoint and lists all the posts", () => {
    ...
    cy.intercept("GET", "/comments/655b65ab227b3e82695769de", (req) => {
      req.reply({
        ...
        body: {
          comments: [
            { _id: 1, message: "first comment", post_id: "655b65ab227b3e82695769de" },
            { _id: 2, message: "second comment", post_id: "655b65ab227b3e82695769de" },
          ],
        },
      });
    }).as("getComments");

    cy.mount(<CommentFeed navigate={navigate} post_id="655b65ab227b3e82695769de"/>);
...
describe("CommentFeed", () => {
  ...
  it("Displays a message for no posts", () => {
    ...
    cy.wait("@getEmptyComments").then(() => {
      cy.get('[data-cy="no-comments-message"]').should('contain.text', "No one has commented yet - be the first!");
